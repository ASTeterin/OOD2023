...
const int MAX_CHOCOLATE_SLICES_COUNT = 5;
...
{
public:
	CLiquor(IBeveragePtr&& beverage, LiquorType syrupType)
		: CCondimentDecorator(move(beverage))
		, m_liquorType(syrupType)
	{
	}
...
protected:
	...
	std::string GetCondimentDescription() const override
	{
		...
		case LiquorType::Jagermeister:
			return "Liquor Jagermeister";
		default:
...
{
public:
	CChocolateSlices(IBeveragePtr&& beverage, unsigned slicesCount)
		: CCondimentDecorator(move(beverage))
	{
		if (slicesCount > MAX_CHOCOLATE_SLICES_COUNT)
		{
			throw std::invalid_argument("Invalid slices count");
		}
		m_count = slicesCount;
	}
	...
	std::string GetCondimentDescription() const override
	{
		return m_count == 0 ? "" : std::to_string(m_count) + " сhocolate slices";
	}
